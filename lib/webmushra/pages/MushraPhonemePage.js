...
function MushraPhonemePage(_pageManager, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler, _language) {
	this.isMushra = true; 
...
MushraPhonemePage.prototype.getName = function () {
  return this.pageConfig.name;
};

MushraPhonemePage.prototype.init = function () {
   var toDisable;
  var td;
  var active; 
...
MushraPhonemePage.prototype.unfreeze = function () {
  this.frozen = false;
  // "Unfreeze" should reset the audio player to play reference.
  this.btnCallbackReference()
  // TODO: change button color when pressed
  var selector = '#unfreezeButton'
  $.mobile.activePage.find(selector)
    .removeClass('ui-btn-a')
    .addClass('ui-btn-b').attr('data-theme','b');
  $.mobile.activePage.find(selector).focus();
  $(selector).attr("active", "true");
  // remove the slider indicating the region
  $("#slider").css("display","none");
  $("#div_lower_limit").css("display","none");
  $("#div_upper_limit").css("display","none");
  // Freeze the canvas in the WaveformVisualizer.
  this.waveformVisualizer.frozen = true;
}

MushraPhonemePage.prototype.render = function (_parent) {
  var div = $("<div></div>");
  _parent.append(div);
  var content; 
  ...
  var tableMid = $("<table id='mainMid'></table>");
  ...
  // another row with the unfreeze button and nothing else.
  var trLoopTwo = $("<tr id='trWsTwo'></tr>");
  tableMid.append(trLoopTwo);
  var tdLoop2 = $(" \
    <td class='unfreezeButton'> \
      <button data-role='button' data-inline='true' id='buttonUnfreeze' onclick='"+ this.pageManager.getPageVariableName(this) + ".unfreeze();'>" + this.pageManager.getLocalizer().getFragment(this.language, 'unfreezeButton') + "</button> \
    </td> \
  ");
  trLoopTwo.append(tdLoop2)
...
MushraPhonemePage.prototype.pause = function() {
    this.mushraAudioControl.pause();
};

MushraPhonemePage.prototype.setLoopStart = function() {
...
MushraPhonemePage.prototype.setLoopEnd = function() {
  var slider = document.getElementById('slider'); 
...
MushraPhonemePage.prototype.btnCallbackReference = function() {
...
MushraPhonemePage.prototype.surpressLoop = function() {
...
MushraPhonemePage.prototype.btnCallbackCondition = function(_index) {
...
MushraPhonemePage.prototype.renderCanvas = function(_parentId) {
...
MushraPhonemePage.prototype.load = function () {
...
MushraPhonemePage.prototype.save = function () {
  this.macic.unbind(); 
...
MushraPhonemePage.prototype.store = function () {
  ...
  for (i = 0; i  < this.ratings.length; ++i) {
    ...
    var ratingObj = new MUSHRAPhonemeRating();
    ratingObj.stimulus = rating.name;
    ratingObj.score = rating.value;
    ratingObj.time = this.time;
    ratingObj.loopstart = this.loop.start;
    ratingObj.loopend = this.loop.end;
